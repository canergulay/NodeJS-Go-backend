...
function GetMyAdvertsRepositary(userid) {
  return AdvertDataSource.GetAdvertsByUserId(userid)
    .then((adverts) => {
      return responser(0, adverts);
    })
    .catch((e) => {
...
function DeleteMyAdvert(advertid, userid) {
  try {
    ...
    Logger(
      e,
      `An unexpected error when trying to delete advert by advertid and userid : ${advertid} ${userid}`
    );
...
module.exports = {
  saveAdvertRepositary,
  getAdvertsByLatLonRepositary,
  GetMyAdvertsRepositary,
  DeleteMyAdvert,
};
